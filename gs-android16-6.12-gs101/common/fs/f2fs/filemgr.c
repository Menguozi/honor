...
#define MAX_RAMFS_SIZE (2048 * 1024 * 1024UL)
...
int start_filemgr_thread(struct f2fs_sb_info *sbi)
{
	...
	sbi->filemgr_thread->filemgr_task =
		kthread_run(shrink_filemgr_func, sbi, "f2fs_filemgr-%u:%u",
			    MAJOR(dev), MINOR(dev));
...
void stop_filemgr_thread(struct f2fs_sb_info *sbi)
{
	...
	wake_up_all(&filemgr_th->filemgr_wait_queue_head);
	kfree(filemgr_th);
	sbi->filemgr_thread = NULL;
}
...
void prink_list_info(void)
{
	...
	printk(KERN_ALERT
	       "[filemgr-prink_list_info] INIT_INODE_LIST SIZE = %d, STAGE1_INODE_LIST = %d, ACTIVE_INODE_LIST LIST SIZE = %d, INACTIVE_INODE_LIST = %d\n",
	...
	list_for_each_entry_safe(ie, next_ie,
				 &filemgr_list.ilist[INIT_INODE_LIST], list) {
	...
	cnt = 0;
	list_for_each_entry_safe(ie, next_ie,
				 &filemgr_list.ilist[STAGE1_INODE_LIST], list) {
	...
	cnt = 0;
	list_for_each_entry_safe(ie, next_ie,
				 &filemgr_list.ilist[ACTIVE_INODE_LIST], list) {
	...
	list_for_each_entry_safe(
		ie, next_ie, &filemgr_list.ilist[INACTIVE_INODE_LIST], list) {
...
struct inode_entry *find_inode_entry(struct filemgr_list *imlist,
				     unsigned int ino)
{
	struct inode_entry *ie;

	ie = xa_load(&imlist->iroot, ino);
	if (ie)
		return ie;
	return NULL;
}
...
int add_to_inode_list(struct inode *inode, int list_idx)
{
	...
	if (ie) {
		...
		ie->inode = inode;
		ie->ino = inode->i_ino;
		xa_store(&filemgr_list.iroot, inode->i_ino, ie, GFP_NOFS);
		list_add(&ie->list, &filemgr_list.ilist[list_idx]);
		filemgr_list.nr_ilist[list_idx]++;
	}
...
void remove_inode_entry(struct inode_entry *ie, int del_list)
{
	if (del_list)
		list_del(&ie->list);
	xa_erase(&filemgr_list.iroot, ie->ino);
	kmem_cache_free(f2fs_filemgr_entry_slab, ie);
}

int shrink_inode_from_inactive_list(struct list_head *evict_list,
				    unsigned long long size)
{
	...
	if (size > 0) {
		list_for_each_entry_safe_reverse(
			ie, next_ie, &filemgr_list.ilist[INACTIVE_INODE_LIST],
			list) {
			if (ie && ie->inode) {
				inode_size = i_size_read(ie->inode);
				list_del(&ie->list);
...
void shrink_inode_from_active_list(void)
{
	...
	list_for_each_entry_safe_reverse(
		ie, next_ie, &filemgr_list.ilist[ACTIVE_INODE_LIST], list) {
		if (ie && ie->inode) {
			fi = F2FS_I(ie->inode);
...
int init_file_manager(void)
{
	...
	filemgr_list.high_watermark_bytes =
		MAX_RAMFS_SIZE * HIGH_WATERMARK_RATIO / 100;
	filemgr_list.low_watermark_bytes =
		MAX_RAMFS_SIZE * LOW_WATERMARK_RATIO / 100;
	filemgr_list.fg_reclaim_watermark =
		MAX_RAMFS_SIZE * FG_RECLAIM_WATERMARK_RATIO / 100;

	spin_lock_init(&filemgr_list.list_lock);
	spin_lock_init(&filemgr_list.fill_lock);

	// stage1 model
	stage1_conf.m_w0 = 512;
	stage1_conf.n_w0 = 26;
	stage1_conf.m_w1 = 200;
	stage1_conf.n_w1 = 512;
	stage1_conf.m_w2 = 2;
	stage1_conf.n_w2 = 200;
	stage1_conf.m_w3 = 2;
	stage1_conf.n_w3 = 2;

	stage1_conf.m_b0 = 512;
	stage1_conf.n_b0 = 1;
	stage1_conf.m_b1 = 200;
	stage1_conf.n_b1 = 1;
	stage1_conf.m_b2 = 2;
	stage1_conf.n_b2 = 1;
	stage1_conf.m_b3 = 2;
	...
	stage1_conf.m_scale = 26;
	stage1_conf.n_scale = 1;
	stage1_conf.m_mean = 26;
	stage1_conf.n_mean = 1;

	// stage2 model
	stage2_conf.m_w0 = 512;
	stage2_conf.n_w0 = 66;
	stage2_conf.m_w1 = 200;
	stage2_conf.n_w1 = 512;
	stage2_conf.m_w2 = 2;
	stage2_conf.n_w2 = 200;
	stage2_conf.m_w3 = 2;
	stage2_conf.n_w3 = 2;

	stage2_conf.m_b0 = 512;
	stage2_conf.n_b0 = 1;
	stage2_conf.m_b1 = 200;
	stage2_conf.n_b1 = 1;
	stage2_conf.m_b2 = 2;
	stage2_conf.n_b2 = 1;
	stage2_conf.m_b3 = 2;
	...
	stage2_conf.m_scale = 66;
	stage2_conf.n_scale = 1;
	stage2_conf.m_mean = 66;
	stage2_conf.n_mean = 1;
...
void destroy_file_manager(void)
{
	...
	for (i = 0; i < NR_INODE_LIST; i++) {
		list_for_each_entry_safe(ie, next_ie, &filemgr_list.ilist[i],
					 list) {
			xa_erase(&filemgr_list.iroot, ie->inode->i_ino);
			list_del(&ie->list);
			kmem_cache_free(f2fs_filemgr_entry_slab, ie);
		}
		list_for_each_entry_safe(ie, next_ie, &filemgr_list.ilist[i],
...
static void print_inode_stat(struct inode *inode, const char *str)
{
	...
	printk(KERN_ALERT
	       "[filemgr-print_inode_stat] %lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%s\n",
	       fi->ac_stat_stage1[0], fi->ac_stat_stage1[1],
	       fi->ac_stat_stage1[2], fi->ac_stat_stage1[3],
	       fi->ac_stat_stage1[4], fi->ac_stat_stage1[5],
	       fi->ac_stat_stage1[6], fi->ac_stat_stage1[7],
	       fi->ac_stat_stage1[8], fi->ac_stat_stage1[9],
	       fi->ac_stat_stage1[10], fi->ac_stat_stage1[11],
	       fi->ac_stat_stage1[12], fi->ac_stat_stage1[13],
	       fi->ac_stat_stage1[14], fi->ac_stat_stage1[15],
	       fi->ac_stat_stage1[16], fi->ac_stat_stage1[17],
	       fi->ac_stat_stage1[18], fi->ac_stat_stage1[19],
	       fi->ac_stat_tl[0], fi->ac_stat_tl[1], fi->ac_stat_tl[2],
	       fi->ac_stat_tl[3], fi->ac_stat_tl[4], fi->ac_stat_tl[5], str);
}
...
static void print_inode_stat2(struct inode *inode, const char *str)
{
	...
	printk(KERN_ALERT
	       "[filemgr-print_inode_stat2] %lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%lu,%s\n",
	       fi->ac_stat_stage1[0], fi->ac_stat_stage1[1],
	       fi->ac_stat_stage1[2], fi->ac_stat_stage1[3],
	       fi->ac_stat_stage1[4], fi->ac_stat_stage1[5],
	       fi->ac_stat_stage1[6], fi->ac_stat_stage1[7],
	       ...
	       fi->ac_stat_stage2[0], fi->ac_stat_stage2[1],
	       fi->ac_stat_stage2[2], fi->ac_stat_stage2[3],
	       fi->ac_stat_stage2[4], fi->ac_stat_stage2[5],
	       fi->ac_stat_stage2[6], fi->ac_stat_stage2[7],
	       fi->ac_stat_stage2[8], fi->ac_stat_stage2[9],
	       fi->ac_stat_stage2[10], fi->ac_stat_stage2[11],
	       fi->ac_stat_stage2[12], fi->ac_stat_stage2[13],
	       fi->ac_stat_stage2[14], fi->ac_stat_stage2[15],
	       fi->ac_stat_stage2[16], fi->ac_stat_stage2[17],
	       fi->ac_stat_stage2[18], fi->ac_stat_stage2[19],
	       fi->ac_stat_stage2[20], fi->ac_stat_stage2[21],
	       fi->ac_stat_stage2[22], fi->ac_stat_stage2[23],
	       fi->ac_stat_stage2[24], fi->ac_stat_stage2[25],
	       fi->ac_stat_stage2[26], fi->ac_stat_stage2[27],
	       fi->ac_stat_stage2[28], fi->ac_stat_stage2[29],
	       fi->ac_stat_stage2[30], fi->ac_stat_stage2[31],
	       fi->ac_stat_stage2[32], fi->ac_stat_stage2[33],
	       fi->ac_stat_stage2[34], fi->ac_stat_stage2[35],
	       fi->ac_stat_stage2[36], fi->ac_stat_stage2[37],
	       fi->ac_stat_stage2[38], fi->ac_stat_stage2[39],
	       fi->ac_stat_tl[0], fi->ac_stat_tl[1], fi->ac_stat_tl[2],
	       fi->ac_stat_tl[3], fi->ac_stat_tl[4], fi->ac_stat_tl[5], str);
}
...
void classify1(struct list_head *list, int nr_inodes)
{
	...
	// do_gettimeofday(&start);
	i_array = (struct inode **)kmalloc(nr_inodes * sizeof(struct inode *),
					   GFP_ATOMIC);
	if (!i_array)
		goto error_out;
	...
	list_for_each_entry(ie, list, list) {
		...
		if (fi->last_access_time != 0)
				...
				fi->last_access_time - fi->create_time;
		else
			fi->ac_stat_tl[5] = 20;
		...
		for (j = 0; j < 20; j++)
			input->data[i][j] =
				fi->ac_stat_stage1[j] * KERNEL_TIMES;
	...
	for (i = 0; i < out->rows; i++) {
		...
		if (out->data[i][0] == 0) {
			...
			F2FS_I(inode)->ac_stat_stage2 =
				kmalloc(sizeof(unsigned long) * 40,
...
void classify2(struct list_head *list, int nr_inodes)
{
	...
	list_for_each_entry(ie, list, list) {
		...
		for (j = 20; j < 60; j++)
			input->data[i][j] =
				fi->ac_stat_stage2[j - 20] * KERNEL_TIMES;
...
ssize_t handle_unevictable_inode(struct inode *inode)
{
	...
	if (filemgr_list.active_used_bytes + inode_size >
	    filemgr_list.high_watermark_bytes) { // 没有空间了
		inode_unlock(inode);
		fill_inactive_list();
	...
	if (filemgr_list.active_used_bytes + inode_size >
	    filemgr_list.high_watermark_bytes) { // 依然没有空间，不管理这个文件了
		ret = -1;
		goto out;
	}
...
void handle_writeback_inode(struct inode *inode)
{
	...
	clear_inode_flag(inode, FI_RAMFS);
...
void handle_inode_by_type(struct list_head *inode_list)
{
	...
	list_for_each_entry_safe_reverse(
		ie, next_ie, inode_list,
		list) { // 遍历处理每一个f2fs_inode_indo的类型
		if (ie && ie->inode) {
			list_del(&ie->list);
			if (is_inode_ftype(
				    ie->inode,
				    INODE_UNEVICTABLE)) { // INODE_UNEVICTABLE
				ssize_t isize =
					handle_unevictable_inode(ie->inode);
				if (isize > 0) { //
					filemgr_list.active_used_bytes += isize;
					filemgr_list
						.nr_ilist[ACTIVE_INODE_LIST]++;
					list_add(
						&ie->list,
						&filemgr_list.ilist
							 [ACTIVE_INODE_LIST]); // 新分类的文件都进入ACTIVE LIST
					nr_mem++;
				} else { // No memory
					handle_writeback_inode(ie->inode);
					remove_inode_entry(
						ie,
						0); // dont manage this type of files
					nr_cln++;
				}

			} else if (is_inode_ftype(ie->inode,
						  INODE_CLEAN)) { // INODE_CLEAN
				dsize += i_size_read(ie->inode);
				clear_inode_flag(ie->inode, FI_RAMFS);
				...
				list_add(
					&ie->list,
					&filemgr_list.ilist
						 [STAGE1_INODE_LIST]); // 新分类的文件都进入ACTIVE LIST
				nr_stage1++;
			} else { // WRITEBACK处理
				fi = F2FS_I(ie->inode);
...
void classify_files(void)
{
	...
	list_for_each_entry_safe_reverse(
		ie, next_ie, &filemgr_list.ilist[STAGE1_INODE_LIST], list) {
	...
	spin_lock(&filemgr_list.list_lock);
	list_for_each_entry_safe_reverse(
		ie, next_ie, &filemgr_list.ilist[INIT_INODE_LIST], list) {
		struct f2fs_inode_info *fi = F2FS_I(ie->inode);
	...
	spin_unlock(
		&filemgr_list
			 .list_lock); // 此时INIT_INODE_LIST的list已经没有数据了，可以解锁了

	// step 4:  处理第一次分类的结果
	if (nr_inode > 0) {
		classify1(
			&inode_list,
			nr_inode); // 进行第一次分类，将数据写到STAGE1_INODE_LIST
		spin_lock(&filemgr_list.fill_lock);
		// handle_inode_by_type(&inode_list);
		spin_unlock(&filemgr_list.fill_lock);
	}
...
void __shrink_files(int is_fg, int is_lock)
{
	...
	if (!is_fg &&
	    filemgr_list.active_used_bytes <
		    filemgr_list
			    .low_watermark_bytes) // 如果是后台，且还没达到水线，就马上返回
		return;

	if (is_fg)
		evict_size =
			filemgr_list.active_used_bytes -
			filemgr_list.fg_reclaim_watermark; // 回收到低水线以下;
	else
		evict_size = filemgr_list.active_used_bytes -
			     filemgr_list.low_watermark_bytes;

	nr_evict =
		shrink_inode_from_inactive_list(&inode_evict_list, evict_size);

	for (; nr_evict; nr_evict--) {
		ie = list_last_entry(&inode_evict_list, struct inode_entry,
				     list);
		if (ie && ie->inode) {
...
int filemgr_unlink(struct inode *dir, struct dentry *dentry, int is_lock)
{
	...
	if (!is_inode_flag_set(
		    target,
		    FI_RAMFS_DELETED)) { // 如果没有被用户删除，那么i_count=3，所以先-1
...
int unlink_file_by_inode(struct inode *inode, int is_lock)
{
	...
	if (next) {
		dentry = hlist_entry(
			next, struct dentry,
			d_u.d_alias); // do not get the reference of this dentry
		if (!IS_ERR(dentry)) {
