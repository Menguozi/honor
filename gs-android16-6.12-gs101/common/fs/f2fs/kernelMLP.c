...
static mat_t my_int_pow(mat_t base, unsigned int exp)
{
    mat_t result = 1;
    int i;
...
static mat_t my_tanh(mat_t xx)
{
    ...
    for(i = 1; i < nr; i+=2)
        sinh_val += my_int_pow(xx, i) / factorial(i);

    for(i = 0; i < nr; i+=2)
        cosh_val += my_int_pow(xx, i) / factorial(i);
...
static mat_t my_exp(mat_t xx)
{
    ...
    for(i = 0; i < nr; i++) {
        exp_val += my_int_pow(xx, i) / factorial(i);
    }
    if(less_zero == 1)
...
struct matrix *matrix_normalize(struct matrix *mat, int axis)
{
    ...
    if(axis == 1) { // 每一个属性进行正则化
        ...
        printk(KERN_ALERT "axis must be 0 or 1!\n");
        matrix_release(ret);
        return NULL;
    }
    return ret;
